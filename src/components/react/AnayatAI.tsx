...
export default function AnayatAI() {
	...
	return (
		<div className="flex h-[600px] flex-col text-sm">
					...
					{messages.length === 0 && (
						<motion.div
							...
							<div className="max-w-xs rounded-lg bg-green-100 px-2.5 py-1.5 text-emerald-700">
								Hi! I'm <span className="font-semibold">Anayat's AI persona</span>. Ask me anything about him or his
			...
			<div className="flex w-full justify-between gap-2 pt-4 text-xs">
				<button
					onClick={() => handleButtonClick("What projects have you worked on?")}
				...
				<button
					onClick={() => handleButtonClick("What is your AI/engineering philosophy?")}
			...
			<div className="flex flex-col items-center gap-3 pt-6">
				<p className="text-body/80 text-center text-sm">
				 <span className="">open to a short conversation?</span> 
				
				</p>
				<a href="https://cal.com/anayat-khan-aj3g4f/15min" target="_blank" rel="noopener noreferrer">
					<button
						type="button"
						data-cal-namespace="15min"
						data-cal-link="anayat-khan-aj3g4f/15min"
						data-cal-config='{"layout":"month_view","useSlotsViewOnSmallScreen":"true"}'
						className="rounded-lg flex gap-2 items-center bg-zinc-200 px-2.5 py-1.5 text-zinc-700 transition duration-300 ease-in-out md:hover:scale-95 md:hover:bg-zinc-300 md:hover:text-zinc-900"
						disabled={isProcessing}>
				...
				{/* <button
					...
					className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-[#4A4E57] px-5 py-2.5 text-sm font-medium text-white shadow-sm transition-all duration-300 hover:bg-[#5a5f6a]">
					<span className="relative flex h-4 w-4 items-center justify-center">
						<HiCalendar className="h-4 w-4" />
					</span>
					<span>Book an intro call</span>
